...
down_revision: Union[str, None] = 'b3faf853662b'
...
def upgrade() -> None:
    # Migrate level data
    op.execute("INSERT INTO levels (id, value, name, created_at, updated_at) SELECT id_niveis, id_niveis, nome_nivel, NOW(), NOW() FROM niveis")

    # Migrate musics data
    op.execute("INSERT INTO musics (id, name, url, user_id, title, created_at, updated_at) SELECT id_musicas, name, url, user_id, title, NOW(), NOW() FROM music_links")

    # Migrate servers data
    op.execute("INSERT INTO servers (id, name, discord_id, created_at, updated_at) SELECT id_servidores, nome_servidor, numero_id_servidor, NOW(), NOW() FROM servidores")
